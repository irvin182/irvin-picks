...
export default function LiveTvDashboard() {
  ...
  const [selectedId, setSelectedId] = useState<number | null>(() => {
    ...
    const saved = window.localStorage.getItem("irvin:selected-live-match");
  ...
  function handleSelectMatch(id: number | null) {
    setSelectedId(id);
    if (typeof window !== "undefined" && id) {
      window.localStorage.setItem("irvin:selected-live-match", String(id));
    }
  }
    ...
    async function loadLive() {
      try {
        ...
        setSelectedId((current) => {
          if (current && realMatches.some((m) => m.id === current)) return current;

          if (typeof window !== "undefined") {
            const saved = Number(window.localStorage.getItem("irvin:selected-live-match"));
            if (saved && realMatches.some((m) => m.id === saved)) return saved;
          }

          return realMatches[0]?.id ?? null;
        });
      } catch (error) {
      ...
      <div className="h-screen p-2 grid grid-rows-[58px_minmax(0,1fr)_34px] gap-2 text-[80%]">
        <div className="relative">
          ...
          <button
            ...
            className="absolute right-3 top-3 rounded-xl border border-green-400/30 bg-green-500/10 px-3 py-1 text-[11px] font-black text-green-300 hover:bg-green-500/20"
          >
            VER TUTORIAL
          </button>
        </div>

        <section className="grid grid-cols-[200px_minmax(0,1fr)_280px] gap-2 min-h-0">
          <MatchSelector
            matches={matches}
          ...
          <section className="min-h-0 grid grid-rows-[190px_minmax(0,1fr)] gap-2">
            <ScoreBoard selected={selected} prediction={prediction} />

            <div className="grid grid-cols-3 gap-2 min-h-0 overflow-y-auto">
              {showTutorial && (
                <Card title="TUTORIAL RÁPIDO" wide>
                  <div className="grid grid-cols-4 gap-2 text-center h-full items-center">
                    <MiniStep n="1" text="Elige partido" />
                    <MiniStep n="2" text="Mira marcador y barra 1X2" />
                    <MiniStep n="3" text="Confirma con stats" />
                    <MiniStep n="4" text="Lee recomendación IA" />
                  </div>
                </Card>
              )}
              ...
              <Card title="POSESIÓN">
                <div className="flex items-center justify-center gap-6 h-full">
                  ...
                  <div className="w-16 h-16 rounded-full border-[12px] border-green-500 border-r-blue-600" />
                  <span className="text-3xl font-black">{awayPossession}</span>
                </div>
              </Card>

              <Card title="TIROS TOTALES">
                <div className="flex items-center justify-between text-4xl font-black mt-5">
                  <span>{homeShots}</span>
                  <span>{awayShots}</span>
                </div>
              ...
              <Card title="1ª MITAD">
                ...
                <div className="mt-3 text-center text-white/60 text-xs font-bold">
              ...
              <Card title="ANÁLISIS AMPLIO DEL PARTIDO" wide>
                <p className="text-white/65 text-xs leading-5">
                  {selected.home} vs {selected.away}. El sistema combina marcador,
                  minuto, tiros, tiros a puerta, posesión y modelo Poisson Live.
                  ...
                  . Usa esta lectura como apoyo estadístico, no como garantía.
                </p>
              </Card>
            </div>
          </section>
...
function Card({
  ...
  return (
    <div
      className={`rounded-xl border border-white/10 bg-[#030b13] p-3 overflow-hidden min-h-[104px] ${
        wide ? "col-span-3" : ""
      }`}
    >
      <h3 className="text-center text-white/50 font-black text-[11px] leading-tight mb-1">
        {title}
      </h3>
      {children}
    </div>
...
function MiniStep({ n, text }: { n: string; text: string }) {
  return (
    <div className="rounded-xl bg-white/[0.04] border border-white/10 p-2">
      <div className="text-green-400 text-xl font-black">{n}</div>
      <div className="text-white/65 text-[11px] font-bold">{text}</div>
    </div>
  );
}
